...
// 페이지 로드 시 서버에서 엑세스 토큰을 가져와서 sessionStorage에 저장
fetch('/getKakaoAccessToken')
    .then(response => response.text())
    .then(kakaoAccessToken => {
        // 받은 카카오 엑세스 토큰을 sessionStorage에 저장
        console.log("kaokao:" + kakaoAccessToken);
        sessionStorage.setItem('kakaoAccessToken', kakaoAccessToken);
        console.log("카카오 엑세스 토큰 저장: ", kakaoAccessToken);
    })
    .catch(error => {
        console.error('엑세스 토큰을 가져오는 데 실패했습니다:', error);
    });
