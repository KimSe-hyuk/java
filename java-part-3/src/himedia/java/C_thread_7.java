...
// ** 스레드 상태
// [NEW]
// 스레드가 생성되고 아직 start()가 호출이되지 않은 상태

// [RUNNABLE]  실행 중 또는 실행 가능한 상태(실행대기)
// -> 스레드를 생성하고 start()를 호출하며 바로 실행되는 것이 아니라
...
// -> 실행 중에 suspend(), sleep(), wait(), join(), I/O block에 의해 일시정지된 상태가 될 수 있다.
// I/O block은 입출력작업에서 발생하는 지연상태를 뜻한다. 사용자의 입력을 기다리는 경우를 예로 들 수 있는데
// 이런 경우 일시정지 상태에 있다가 사용자가 입력을 바치면 다시 실행대기 상태가 된다.
...
// 지정한 시간이지나고 나면, 자동적으로 다시 실행대기 상태가 된다.
...
class ThreadEx_yied extends Thread{
    private String name;

    public ThreadEx_yied(String name) {
        this.name = name;
    }
    ...
    public void run() {
        for (int i = 0; i < 5; i++) {
            ...
            // 현재실행 중인 스레드가 다른 스레드에게 실행 기회를 양보
            try {
                Thread.sleep(500);
            } catch (InterruptedException e) {
                throw new RuntimeException(e);
...
//        }
        ...
        // t1이 가장 늑게 종료되어야 하는데 결과에서는 제일 먼저 종료가 될 떄가 있다.
    ...
    // 스레드가 sleep(), whit(), join()에 의해 '일시 정지 상태(WAITING)'에 있을 떄,
    // 해당 스레등에 대해 interrupt()를 호출하면, sleep(), whit(), join()에서
    // InterruptedException이 발생하고 스레드는 '실행 대기 상태로'바뀐다.
    ...
    public static void exam3(){
        ThreadEx_yied t1 = new ThreadEx_yied("Thread 1");
        ThreadEx_yied t2 = new ThreadEx_yied("Thread 2");
