...
// List 중복된 요소를 허용, 인덱스를 기반으로 요소에 접근한다.
...
// - contatins(Object o) : 리스트에 특정 요소가 포함되어 있는지 확인한다.
// - clear() : 리스트의 모든 요소를 제거한다.

// * 요약
// - ArrayList는 배열 기반의 리스트로, 인덱스를 통한 빠른 접근이 가능하지만,
// 중간 삽입/ 삭제가 느리다.           //삽입이나 삭제시 뒤에 값들을 밀어야한다.
// - LinkedList는 노드 기반의 리스트로, 삽입/삭제가 빠르지만 인덱스를 통한 접근이 느리다.   // 이전 주소 다음 주소를 같기에 삽입이나 삭제시 다음 주소를갖 고오기만 하면 됨  데이터 접근시 다음 배열처럼 차례대로가 아니라 처음부터 전부 돌아야해서 느리다.
// Stack은 후입선출(Last In First Out)구조를 가지며, 요소를 스택에 추가하고 제거하는데 사욛된다.


import java.util.*;

public class A_collectins_list {
    // 1. ArrayList
    public static void exam1(){
        ...
        //리스트의 모든 요소 출력
        System.out.println("List elemet"+ list);
    ...
    // - 주요 메서드 : push() 요소 삽입, pop() 요소 제거, peek() 맨 위의 요소를 확인 , empty() 현재 스택이 비었으면 true, 아니면 false를 반환합니다.
    ...
    public static void exam3(){
        ...
        while(iterator.hasNext()){
            String element = iterator.next();
            System.out.println("Iterator elements : " +element);
        }
