...
// 이 메모리는 메서드가 작업을 수행하는 동안 지역변수(매개변수 포함)들과 연산의 중간결과 등을 저장하는데 사용한다.
// 그리고 메서드가 작업을 마치면 할당되었던 메모리 공간은 반환되어 비워진다.

// - 메서드가 호출되면 수행에 필요한 만큼의 메모리를 스택에 할당받는다.
// - 메서드가 수행을 마치고나면 사용했던 메모리를 반환하고 스택에서 제거된다.

// * 기본형 매개변수 : 변수의 값을 읽기만 할 수 있다.(read only)메모리에 값을 가짐 ,기본형변수 8개
// * 참조형 매개변수 : 변수의 값을 읽고 변경할 수 있다.(read & write)메모리에 주소를 가짐 값을 가지지 않음,나머지 변수
