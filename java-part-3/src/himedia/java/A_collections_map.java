...
// * Map
// 키와 값의 싸응 저장하는 자료구조를 나타낸다
// Map은 중복된 키를 허용하지 않으며, 각 키는 하나의 값에 매핑된다.
// Map인터페이스를 구현하는 대표적인 클래스에는 HashMap, TreeMap, ListMap 등이 있다.
...
// - get(Object key) : 키에 대응하는 값을 반환한다. 해당 키가 존재하지않으면 null을 반환한다.
// - remove(Object key) : 특정 키에 대응하는 키-값 싸을 제거한다.
// - containsKey(Object key) : 맵에 특정 키가 포함되어 있는지 확인한다.
// - containsValue(Object value) : 맵에 특정 값이 포함되어 있는지 확인한다.
// - size() : 맵에 저장된 키-값 싸의수를 반환한다.
