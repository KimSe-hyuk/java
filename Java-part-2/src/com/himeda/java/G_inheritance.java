...
// 필요에 따라 추가적인 속성이나 메서드를 정의할 수있다.
// 3. extends 키워드
// 자식 클래스가 부모 클래스를 상속받기 위해 사용된다.
// 4. 메서드 오버라이딩(Method Overriding)
// 자식 클래스가 부모 클래스의 메서드를 재정의하여 사용할 수 있다.
// 이때 부모 클래스의 메서도와 동일한 이름, 반환 타입, 매개변수를 가져야한다.
// 5. 다형성(Polymorphism)
// 상속을 통해 자식 클래스는 부모 클래승의 형태로 사용될 수 있으며, 이는 코드의 유연성과 확장성을 높인다.

// * 오버라이딩(Overriding)
// 자바에서 상속받은 부모 클래승의 메서드를 자식 클래스에서 재정의하여 사용하는 기법이다.
...
// 5. 다형성
// 오버라이딩은 다형성의 기반이 된다. 부모 클래스 타입의 참조 변수가 자식 클래승의 객체를 참조할 떄,
// 오버라이딩된 메서드가 호출되어 자식 클래스 메서드가 실행된다.
// 6. 단일 상속
// 자바는 '다중 상속'ㅇ,ㄹ 허용하지 않고, 단일 상속만 허용한다.
