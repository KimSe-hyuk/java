...
class OuterClass{
    ...
    static class StaticNestedClass{
        void display(){
            ...
            //System.out.println("Inner variable : " + instanceVar);
        }
    }
    //비정적 중첩 클래스
    class InnerClass{
        void display(){
            System.out.println("Static variable : " + staticvar);//정상작동
            System.out.println("Inner variable : " + instanceVar);//정상작동
        }
    }
}
public class D_static_class {
    ...
    // 정적 중첩 클래스를 사용하는 것이 효율적이다.

    // [비정적 중첩 클래스]
    // 바깥 클래스의인스턴스와 연결되기 때문에, 인스턴스당 추가적인 메모리를 소비할 수 있다.
    // 따라서 이 클래스가 바깥 클래스의 인스턴스 데이터에 접근해야하는 경우에만 사용하는 것이 좋다.
