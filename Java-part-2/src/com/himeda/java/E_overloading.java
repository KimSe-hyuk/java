...
// 즉 매개변수 목록이 동일하지만 반환타입만 다른 메서드는 오버로딩으로 간주되지 않으며,
// 컴파일 오류가 발생한다.

// 오버로딩의장점
// - 코드의 가독성 : 같은 이름의 메서드를 사용해 다양한 입력을 처리할 수 있어 코드가 직관적이다.
// - 유지복수 용이성 : 관련된 작업을 수행하는 메서드를 같은 이름으로 그룹화하여 코드를 유지 관리하기 쉽게 한다.
// - 다형성 : 컴파일 시점에서 적절한 메서드를 선택하는 컴파일타입 다형성을 제공한다.

// 오버로딩은 자바에서 메서드를 유연하고 효율적으로 사용하는 중요한 기법이다.
// 동일한 작업을 수행하되, 입력 형탱에 따라 조금씩 다른 동작을 요구할 때 유용하게 사용된다.
