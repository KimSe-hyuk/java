...
// * 생성자(Constructor)
// 객체가 생성될 떄 호출되는 특수한 메서드이다.
// 생성자는 객체의 초기 상태를 설정하는데 사용되며,
// 클래스 이름과 동일한 이름을 가져야한다.

//생성자는 반환 타입이 없으며, 객체가 생서될 때 자동으로 호출된다.
...
// - 객체를 생성할 떄 인수를 전달받아 필드를 초기화하는 생성자이다.
// - 여러 개의 생성자를 오버로딩하여 다양한 초기화 방법을 제공할 수 있다.
/*// 만약 매개변수가 있는 생성자가 하나라도선언되어 있으면
        // 기본 생성자는 자동으로 생성되지 않는다. 기본 생성자를 명시적으로 정의해야 함*/
// 3. 복사 생성자(Copy Constructor)
// - 동일한 클래스의 다른 객체를 복하하여 새로운 객체를 생성하는 생성자이다.
...
public class F_construtor {
    public static void main(String[] args) {
        ...
        // new F_person2() // 만약 매개변수가 있는 생성자가 하나라도선언되어 있으면
        //        // 기본 생성자는 자동으로 생성되지 않는다. 기본 생성자를 명시적으로 정의해야 함

        F_person2 p2 = new F_person2("John2",13);
        p2.display();
